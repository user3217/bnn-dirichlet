...
import models, utils, image_utils


def make_epoch_step(log_prob_fn, x_train, y_train, n_class, augment, batch_size,
           max_temp, max_lr, n_epochs, momentum_decay=0.98):
    """
    ...
    def step_epoch(epoch, args):
        ...
        # - lr: ‾‾‾\_
        # - temp.: __/‾‾
        lr = max_lr * jnp.where(epoch < n_epochs/2, 1, jnp.sin(jnp.pi*(epoch/n_epochs)))
        T = max_temp * jnp.clip(3*(epoch/n_epochs) - 1, 0, 1)
        h = jnp.sqrt(lr / n_train)

        # augment data
        x, y = x_train, y_train
        if augment: x = image_utils.augment(key_augment, x)
