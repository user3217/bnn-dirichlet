...
from run import run
kwargs = dict(model_name='resnet20', ds_name='cifar10', augment=True, batch_size=125)
...
for std, lr in product(stds, lrs):
    run(normal_prior_scale=std, T=0, lr=lr, seed=-1, **kwargs)
...
temps = (0, *np.geomspace(0.0001, 1, 11))
seeds = range(2)
for seed, temp, (std, lr) in product(seeds, temps, zip(stds, lrs)):
    run(normal_prior_scale=std, T=temp, lr=lr, seed=seed, **kwargs)
